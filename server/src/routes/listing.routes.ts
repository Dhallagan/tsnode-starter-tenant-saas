...
export class ListingRoutes extends BaseRoute {
    ...
    initRoutes() {
        ...
        this.router.get('/listings/:id/relations', Authentication.isAuthenticated, (req, res, next) => this.listingController.getListingWithRelations(req, res).catch(next));
        this.router.get('/listings', (req, res, next) => this.listingController.getAllListings(req, res).catch(next));
